...
- **ESC / P** — Pause menu (P is an alternate bind while playing)
- **M** — Mute / unmute all audio
- **R** — Reroll upgrade choices (level-up, one reroll — two from level 10+) / Restart (game over)
- **1/2/3** — Quick-select upgrades on level-up

## Features

### Combat
- Auto-aim primary weapon with laser bolt visuals and trail effects, plus a target reticle that marks the enemy you're currently locked onto — the reticle bleeds green on a healer, purple on a necromancer, orange-red on an exploder, and gold on an elite so you can read a priority target at a glance. The auto-aim also **weights toward support enemies** — a healer or necromancer is treated as ~20% closer than it really is, so your fire favors shutting down the enemy that's mending or refilling the swarm without ever leaving a point-blank threat unanswered
...
- Critical hits (15% base chance, ★ damage text, orange screen flash, crisp crit ping + extra screen shake so they land hard)
- Kill streaks grant an escalating combo damage bonus (up to +45%) shown live on the streak banner, with named tiers that keep climbing — DOUBLE KILL, TRIPLE KILL, MULTI KILL, KILLING SPREE, RAMPAGE, UNSTOPPABLE, DOMINATING, GODLIKE, and finally LEGENDARY (with a running counter past that); the streak window is a forgiving 2.5s so combos are easier to sustain, and every screen shake hits a little harder while a streak is live so the whole run physically escalates as it heats up
- Laser bolts grow chunkier and longer as you stack damage, so a heavy Power Shot build *looks* as hard-hitting as it plays
- Phase Dash with afterimage trail, invincibility (plus a brief i-frame grace window past the dash for a reliable escape), contact damage scaling with speed, and audio-visual ready cue — carving through a pack now lands a meaty thud + a small camera kick the first time the dash connects; a slightly shorter base cooldown (1.25s) keeps you mobile. Phase Dash **recharges 50% faster while critically wounded** (under 25% HP), and **defeating a boss refills all your banked dash charges**, so your escapes are ready when you need them most
- Ultimate ability with screen-clearing AoE, multi-ring VFX, damage scaling with upgrades, and a stronger outward knockback that shoves enemies away for a panic-button "clear space" feel (bosses resist most of the push) — grants a reliable i-frame window on cast (0.7s) so pressing Q while swarmed actually buys an escape, with a base cooldown of 7.0s. Clearing a **flawless (no-damage) wave also refunds 40% of the Ultimate's remaining cooldown**, so clean aggressive play gets the panic button back sooner
- Gravity Well isn't just crowd control — slowed enemies also take +25% damage, so the slow field has a real offensive payoff
- Built-in comeback systems keep a bad run alive: **Last Stand** softens incoming hits by an extra ~15% while you're critically wounded (under 20% HP), enemies drop **health orbs far more often the lower your HP falls**, and a **flawless wave heals you ~12%** on top of the XP bonus
...
### Weapons & Upgrades (36 upgrades)
- **Primary Stats**: Rapid Fire, Power Shot, Overcharge (+18% damage **and** +10% projectile speed per stack, up to 3 — an offensive hybrid), Overdrive (+12% fire rate **and** +8% projectile speed per stack, up to 2 — a cadence/velocity hybrid for a fast, flat-shooting build), Marksman (+12% damage **and** +18% projectile speed per stack, up to 3 — a precision hybrid for hitscan-feeling shots), Fortify (+20 max HP and heal), Swift, Multi-Shot, Magnet
...
- **Combat**: Piercing Rounds, Ricochet (bolts now phase through enemies and keep bouncing off the walls until their bounces are spent — they ricochet around the arena instead of dying on first contact), Shatter Point, Critical Surge (each stack adds +5% crit chance **and** +0.15× crit damage), Velocity Rounds, Executioner (bonus damage to enemies under 40% HP, with red execution flash), Adrenaline (outgoing damage rises as *your* HP falls — up to +40% near death, a risk/reward boost that pairs with Vampire and dash i-frames)
- **Defense**: Nano Shield (with visible blue shield ring), Reactive Plating (+12% max HP with an instant heal **and** +6% incoming damage reduction per stack, up to 2 — a defensive hybrid), Titanium Plating (+18% max HP **and** an instant heal per stack, up to 3 — a percentage boost that keeps scaling into late waves where flat Fortify falls off), Regeneration (+1.6 HP/sec per stack, with heal particle VFX), Vampire (3 HP lifesteal per kill, up to 3 stacks), Bloodlust (+2 HP lifesteal per kill **and** +12% fire rate per stack, up to 2 — a sustain/aggression hybrid that turns a fast-firing build into a self-healing one), Guardian Angel (one-time death save — a fatal hit instead leaves you at 45% HP with a protective cyan burst, a longer moment of invincibility, and a shockwave that shoves the surrounding swarm back so the revive actually buys room to recover), Thorns (reflect 25%/50% of contact damage back into enemies that touch you — a brawler pick for tanky builds)
...
- **Utility**: Gravity Well (slowed enemies also take +25% damage **and** deal 30% less contact damage while caught in the well — offensive *and* defensive value), Greed (+20% XP from all sources, up to 3 stacks), Scavenger (health orbs drop more often **and** heal for more — a sustain pick for long runs, up to 2 stacks)
...
- **Exploder** — rushes player and arms a short fuse on contact (urgent beep + a hard danger ring) rather than detonating instantly, giving you a window to Phase Dash clear of the blast; still chain-reacts with nearby exploders and detonates if killed. The blast damages **any** nearby enemy across its full radius (with distance falloff), so baiting an exploder into a pack is a real crowd-clear play and chains trigger reliably (blast damage scales with wave)
- **Teleporter** — blinks to random positions near the player, unpredictable (blink frequency scales with wave)
- **Healer** — hangs back behind the swarm and pulses healing into nearby wounded enemies (green ring + shimmer telegraph the pulse, mended enemies glint green) — a priority target that undoes your chip damage if left alive (wave 4+). Killing one plays a distinct "power-down" cue and is **guaranteed to drop a health orb**, so hunting the mender pays you back the HP it was denying you
- **Skeleton Golem** (Boss, every 5th wave) — multi-phase with slam, rock throw with ground target indicator (3-rock spread when enraged, fiery trail particles, distinct throw SFX), charge (with knockback, ground impact VFX), and enrage below 30% HP. Boss speed scales with wave progression. A long fight also hands you a lifeline — the golem drops a generous heal orb the first time it crosses 50% HP and again when it enrages — so the duel isn't a pure war of attrition. Defeating a boss awards bonus XP with dramatic gold flash
- **Elite variants** (wave 6+) — any regular enemy has a small, wave-scaling chance to spawn as a tougher, higher-value Elite: ~1.9× HP, faster, hits harder, drops ~2.2× XP and is much likelier to drop a heal orb. Each elite also rolls one of three **flavors**, flagged by its rim/chevron color so the threat reads at a glance: **Armored** (steel-blue — a slow, spongy wall you have to commit damage to), **Swift** (cyan — a fragile sprinter that punishes standing still), or **Vampiric** (crimson — heals itself off a slice of every contact hit it lands, so leaving it alive drags the fight out). All elites carry a larger build and an always-on HP bar so they read as priority/reward targets. **Splitting** — every elite bursts into two fresh minions when killed, so ignoring one lets it refill the swarm (the split is capped to respect the alive-enemy limit, and the spawned minions are never elite so there's no runaway)
...
### HUD & UI
- Corner minimap radar — plots the swarm relative to you (you're the cyan dot at center), with bosses shown in orange, elites in gold, and priority support enemies flagged too (healers in green, necromancers in purple); off-radar contacts clamp to the edge so a distant threat still shows its bearing
...
- Boss HP bar with wave-specific label ("SKELETON GOLEM — WAVE X"), a live numeric HP readout (e.g. "62%  ·  1240 / 2000 HP") so the fight reads as a DPS check, color shift (green → yellow → red), and enrage warning ("ENRAGED" + pulsing red)
...
- Pause menu (resume/restart/mute/quit) with a live MUTE/UNMUTE toggle button, an at-a-glance run summary plus a full build sheet (damage, fire rate, projectiles, crit, DR, regen, lifesteal, XP bonus, Guardian status), your saved best run (with an "ON PACE FOR A RECORD" flag when the current run is beating it), and a compact controls reference (WASD / SPACE / Q / ESC / P / M)
- Upgrade reroll — one per level-up screen (two from level 10+, via button or R key) swaps all three choices
...
- Guardian Angel death-save banner (a bright "★ GUARDIAN ANGEL — SAVED ★" callout + cyan screen flash when the one-per-run revive triggers)
- Game over screen with stats (damage dealt/taken, kills/min, DPS, best kill streak, total dashes, avg time/wave), performance rating (RECRUIT → LEGENDARY → APEX at wave 30+), power level summary (GEARING UP/ARMED UP/FULLY LOADED/MAXED OUT), build summary, and restart button
- Victory sting plays on reaching wave 10, 15, 20, 25, and 30 milestones
- Victory sting + milestone banner plays on reaching wave 10 (VETERAN), 15 (ELITE), 20 (LEGENDARY), 25 (MYTHIC), and 30 (APEX)
...
- Post-game performance rating (RECRUIT → LEGENDARY → APEX based on waves survived)
...
- **Agency, reward & baseline-feel pass** — More control on the level-up screen and a snappier baseline. A new **Overdrive** upgrade (a cadence/velocity hybrid — +12% fire rate and +8% projectile speed per stack, up to 2) widens the build tree, and you now get a **second reroll from level 10 on** so a deep-run screen can fish harder for the piece your build needs. Clean play is rewarded more: a **flawless (no-damage) wave now also refunds 40% of the Ultimate's remaining cooldown** on top of its XP and heal, so aggressive no-hit runs get their panic button back sooner. The **Ultimate clears more room** (a slightly wider base radius and a stronger non-boss knockback) and its **cast i-frame window is longer** (0.7s, up from 0.55s) so pressing Q while swarmed more reliably buys an escape. **Guardian Angel** revives you a touch higher and safer (**45% HP** with a longer invulnerability window). Baseline feel: **base crit chance up to 15%** (from 12%) so early fights land more of the crisp crit pings, the **base Phase Dash cooldown is snappier** (1.25s, from 1.4s), and **base pickup range is wider** (5.2) so early XP and heal orbs aren't stranded before you grab Magnet. QoL: **P now works as an alternate pause key** alongside ESC.
- **Elite flavors, build variety & focus-fire pass** — Elites are no longer all the same gold sponge: each now rolls one of three **flavors** flagged by rim/chevron color — **Armored** (steel-blue, a slow tanky wall), **Swift** (cyan, a fragile sprinter), and **Vampiric** (crimson, heals itself off every contact hit it lands) — so a gold target's threat reads at a glance and demands a different response. Two new upgrades widen the tree further: **Bloodlust** (a sustain/aggression hybrid — +2 HP lifesteal per kill and +12% fire rate per stack, up to 2) and **Marksman** (a precision hybrid — +12% damage and +18% projectile speed per stack, up to 3). Focus-fire help: the **auto-aim now weights toward support enemies** (healers/necromancers are treated as ~20% closer) so your fire favors shutting down the swarm's mender/summoner, and the **radar flags them too** (healers green, necromancers purple) alongside bosses and elites. Comeback: a long **boss fight now drops a generous heal orb at its 50% and enrage beats** so the duel isn't a pure war of attrition, and **health orbs rush to you the moment you drop below 25% HP** so a clutch drop isn't stranded across the arena. Feel: **screen shake scales with your active kill streak** (still hard-capped) so a run physically escalates as it heats up, and the streak banner gains higher tiers — **DOMINATING, GODLIKE, LEGENDARY**. Utility: **Gravity Well now also cuts slowed enemies' contact damage by 30%**, giving the field defensive value on top of its damage amp.
- **Build variety, priority-target & comeback pass** — Two new upgrades widen the build tree: **Overcharge** (an offensive hybrid — +18% damage and +10% projectile speed per stack, up to 3) and **Reactive Plating** (a defensive hybrid — +12% max HP with an instant heal and +6% incoming damage reduction per stack, up to 2). Support enemies now pay you back for focusing them: **Healers are guaranteed to drop a health orb** on death, and killing a **Healer or Necromancer** plays a distinct "power-down" cue so shutting down the swarm's mender/summoner reads as a real win. **Surviving a boss now also refills all your dash charges**, so you leave a long golem fight with your escapes ready instead of on an empty meter. Pacing & feel: when only a couple of enemies are left alive they **surge ~40% faster** toward you so end-of-wave cleanup isn't a slow chase, and an **Exploder's blast now damages nearby enemies across its full radius** (with falloff) — baiting one into a pack is a genuine crowd-clear and chain reactions trigger far more reliably. Comeback mobility: **Phase Dash recharges 50% faster while critically wounded** (under 25% HP). Balance: **Fortify buffed to +20 max HP** (from +15) so the earliest defensive pick keeps pace early, and **Orbital Guard's ring is a touch wider** (radius 2.5 → 2.8).
- **Survivability, clarity & feedback pass** — A light balance-and-polish round. Defensive picks reach cleaner ceilings: **Nano Shield now caps at −50% incoming damage** (was −48%) and a maxed **Coolant reaches −45% Ultimate cooldown** (was −40%), so investing all the way into either pays off. **Guardian Angel now revives you at 40% HP** (up from 35%) for a slightly more survivable one-per-run save. Comeback pacing: **leveling up heals ~9% max HP** (up from ~8%), the **between-wave breather runs a touch longer** (1.35s invuln) and its **wave-clear heal scales to ~10% max HP**, and a **flawless wave pays out more bonus XP** deep into a run. Feedback: **mid kill-streak milestones (5 and 8) now land a camera kick + shake** instead of silently passing, and an **Executioner finisher** (the execute bonus landing a killing blow) punches the camera so the "finish the wounded" payoff feels as weighty as it sounds. Clarity: the **Gravity Well** card now spells out its **+25% damage-taken** payoff and shows its slow at the real capped value instead of overstating it at three stacks.
- **Payoff & pacing tune** — A round of generous, run-friendly tuning. **Executioner's window widened to enemies under 40% HP** (from 35%) so the finisher closes wounded packs faster, and **Giant Slayer now grants +30% boss damage per stack** (up from +25%) so an under-geared slayer build can still bring a golem down. The **kill-streak combo ceiling is up to +45%** (from +40%) and ramps a hair faster, and **Multi-Shot's fan is tighter** (12° total) so a stacked build lands more of its pellets on target. The **Ultimate's late-game blast radius scales a little further** so the panic button stays relevant when waves pack in. Comeback & sustain: **leveling up now heals ~8%** max HP and a **flawless wave heals ~12%** (both up a touch), **bosses drop three health orbs** on death instead of two, and **Elites are more likely to cough up a heal**. Health orbs also **heal a little more** (16% max HP, up from 14%) and **Regeneration ticks for +1.6 HP/sec** per stack (up from +1.3) so the sustain picks hold up better deep into a run. Onboarding: the **first level-up now lands at 62 XP** (down from 70) so a fresh run gets its opening upgrade sooner.
- **Reward, comeback & juice pass** — A round of pacing and feel polish aimed at making big moments land and keeping shaky runs alive. **Surviving a boss now heals you ~25% max HP** (with the wave-heal chime) so a golem kill is a real breather before the swarm resumes, and a boss fight gets a **halfway-point beat** — a shake + zoom-kick the first time the boss drops past 50% HP, giving the long fight a clear midpoint before its sub-30% enrage. **Leveling up now also patches you up** a little (~6% max HP), so climbing the XP curve smooths the difficulty instead of only handing over an upgrade. Comeback help: the **kill-streak window stretches by a second while you're critically wounded** (under 35% HP) so a desperate low-HP push can keep a hard-won combo — and its damage bonus — alive. **Kill milestones** (100/500/1000…) now land a camera punch + shake on top of their chime, and **clutch health-orb grabs** (picked up near death) get a bigger zoom-kick and a distinct relief chime. Feel: **taking a contact hit now shakes toward the attacker** so damage reads directionally, and **carving through 3+ enemies in one Phase Dash** earns a scaling zoom-kick so a satisfying pack-slice hits with weight. Cracking a **gold Elite now bursts a small cluster of XP orbs** (same total XP, richer payoff), and **Ricochet bolts live longer** so their wall bounces reliably land instead of expiring mid-flight.
- **Readability & deep-run pass** — A batch of clarity, feedback and end-game polish. The **boss HP bar** now shows a live numeric readout (`62%  ·  1240 / 2000 HP`) so a golem fight reads as a real DPS check instead of a shrinking bar. Reaching **wave 30** now lands its own **APEX** milestone banner + victory sting (deep runs past MYTHIC were previously unmarked), and the game-over screen adds a matching **APEX** top performance tier. **Guardian Angel** finally has a HUD tell — cheating death pops a bright `★ GUARDIAN ANGEL — SAVED ★` banner + cyan screen flash so the clutch revive can't be missed in the chaos. A massively overkilled enemy now gets a deep "obliterated" **finisher thud** on top of its bigger death pop, and the kill-streak audio sting extends to a **30-kill** milestone. The **pause menu** shows your saved best run and flags when the current run is on pace to beat it. Card clarity: **Vampire** now reads its true +3 HP/kill, **Executioner** spells out its "under 35% HP" window, and the **Coolant** preview shows the real (multiplicative, floored) cooldown reduction instead of a misleading flat number.
- **Build variety, elite payoff & balance pass** — Two new upgrades: **Frenzy** (a hybrid aggression pick — +15% fire rate and +8% move speed per stack, up to 3) and **Coolant** (−12% Ultimate cooldown per stack, floored so a maxed stack still leaves a real gap). **Elite kills feel earned now** — cracking a gold elite lands an extra screen shake + camera punch and its own weighty death flourish, on top of the split it already triggers, so a high-value target reads as a real payoff instead of a plain minion pop. The **Ultimate coming off cooldown** now pops a subtle world-space pulse to match its ready-chime, mirroring the dash-ready ring. Balance: **Healer** mends trimmed from 18% to 14% max HP so packed waves are less of a grind, **Vampire** lifesteal bumped to +3.0 HP/kill per stack, and **Giant Slayer** raised to +25%/stack so bosses feel less spongy for a slayer build. Added more **kill-milestone** celebrations (50, 750, 1500) and clarified the **Executioner** card to spell out its "below 35% HP" threshold.
